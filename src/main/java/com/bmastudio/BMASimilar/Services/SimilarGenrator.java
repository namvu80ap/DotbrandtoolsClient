...
public class SimilarGenrator {
	 ...
	 */
	private static final int CHANGE_ONE_ACCESSIBLE = 5;
	...
	private static final int CHANGE_TWO_TYPOS_ACCESSIBLE = 5;
	...
	public static Map<String, Integer> runCompare( Set<String> list, String word ) {
		...
		Map<Double, Integer> cacheList = new HashMap<Double, Integer>();
		
		for (String string : list) {
			
			double cacheScore = StringUtils.getLevenshteinDistance(word, string);

			Integer score = cacheList.get(cacheScore);

			if ( score == null || score == 0) {
				score = CompareNIST.howConfusableInt(word, string);
				cacheList.put(cacheScore, score);
			}

			returnList.put( string, score);
		}

		returnList = BmaSimilarList.sortHashMapByValues(returnList);
		*/
		...
		for (String string : list) {
			...
			if( isAll ){
				...
				if ( score == null ) {
					score = CompareNIST.howConfusableInt(word, string);
					cacheList.put(cacheScore, score);
					if( score >= threshold ) returnList.put( string, score);
				}
